...
| `includes/class-query.php` | Modified | ids branch passes `'type' => array_merge( array_keys( wc_get_product_types() ), array( 'variation' ) )` so variations round-trip (task 1.3, D4) |
...
1. **Include + limit truncation (pre-existing, shared with legacy `ids`)**: `CWC_Query::query()` always sets `'limit' => 8` (default), and `WC_Data_Store_WP::get_wp_query_args()` maps `limit` → `posts_per_page` with no special-casing for `include` (verified in WC source). So a manual-ID list longer than 8 renders at most 8 — identical behavior to the legacy `ids` attribute (PQ-2). The wp-env acceptance case (`products="12,7,3"`) is unaffected. Not changed in this slice per design contract (`query(['ids' => products])`); verify phase should decide whether to surface/limit-bump.
...
4/4 Phase 1 tasks complete. Ready for review (PR #1, do not merge until reviewed).