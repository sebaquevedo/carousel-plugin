...
- [ ] 2.2 Create `includes/class-shortcode.php` — `add_shortcode('cwc_carousel', [$this,'render'])`, `shortcode_atts(['title'=>'','count'=>8,'ids'=>''])`, `sanitize_text_field(title)`, `absint(count)`, ids parse per 2.1; `CWC_Query::query() → CWC_Renderer::render()`; returns string, never echoes. (SC-1, SC-2, SC-3)
